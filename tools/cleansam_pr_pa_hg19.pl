...
# 动态获取脚本所在目录的绝对路径
my $script_dir = dirname(abs_path($0));  # $0 是当前脚本名称，abs_path 解析为绝对路径

# 设置 $gmapdir 为脚本所在目录下的 gmap/bin
my $gmapdir = "$script_dir/gmap/bin/";  # 自动拼接路径
my $database='hg19';                ###type your database name here;
my $order='';
my $strand='';
my $chr='';
my $cmd='';
